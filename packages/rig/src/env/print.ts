...
import { createColors } from "../terminal/colors.js"
import { ENV_META, type EnvMeta, type FieldMeta } from "./define.js"
...
export function formatConfig(config: unknown, name?: string): string {
  ...
  for (const [section, fields] of sections) {
    ...
    for (const field of fields) {
      ...
      lines.push(
        `${colors.gray(BOX_MID)} ${indent}${colors.gray(key)}${padding}${formatted}`,
      )
    }
  }
