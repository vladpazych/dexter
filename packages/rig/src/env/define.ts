/**
 * App-owned configuration from explicit values.
 *
 * Rig keeps the schema layer separate from the public namespace API.
...
export const ENV_META = Symbol.for("rig.env.meta")
...
function coerceInput(
  ...
  return coerce(String(value), type, values)
}
...
function resolveConfigInternal<const S extends EnvSchema>(
  ...
  function lookupValue(envName: string): {
    ...
    source: EnvFieldReport["source"] | undefined
  } {
    const runtimeValue = runtimeEnv[envName]
    if (runtimeValue !== undefined) {
      return { raw: runtimeValue, source: "input" }
    }
    ...
    for (const [key, def] of Object.entries(obj)) {
      if (isField(def)) {
        ...
        if (raw === undefined) {
          ...
          if (field.hasDefault) {
            if (def.default === undefined) {
              result[key] = undefined
              reportFields.push(
                createFieldReport(fieldPath, field, "default", undefined),
              )
              continue
            }

            try {
              const value = coerceInput(def.default, field.type, def.values)
              result[key] = value
              reportFields.push(
                createFieldReport(fieldPath, field, "default", value),
              )
            } catch (msg) {
              errors.push(`${def.env}: invalid default (${String(msg)})`)
              result[key] = undefined
              reportFields.push(
        ...
        try {
          ...
          result[key] = value
          reportFields.push(
            createFieldReport(fieldPath, field, source ?? "input", value),
          )
        } catch (msg) {
          errors.push(`${def.env}: ${msg}`)
          result[key] = undefined
          reportFields.push(
            createFieldReport(fieldPath, field, source ?? "input", raw),
          )
        }
      } else {
